...
class DataWorker(QObject):
    ...
    def run(self):
        ...
        # Obtener las reservas de restaurantes
        if tripulante_ids:
            restaurantes_query = session.query(
                TripulanteRestaurante.tripulante_id.label("Tripulante_ID"),
                TripulanteRestaurante.tipo_comida.label("Tipo_Comida"),
                ...
                .filter(TripulanteRestaurante.tripulante_id.in_(tripulante_ids))

            # Aplicar filtro por ciudad
            if self.ciudad and self.ciudad != "Ciudad":
                restaurantes_query = restaurantes_query.filter(func.trim(func.lower(Restaurante.ciudad)) == self.ciudad.strip().lower())

            restaurantes = restaurantes_query.all()
        else:
            restaurantes = []

        # Mapear tipos de comida en español a inglés
        tipo_comida_map = {
            "Almuerzo": "Lunch",
            "Cena": "Dinner",
            "Caja Almuerzo": "Box Lunch"
        }

        # Procesar las reservas para clasificar por tipo de comida
        reservas = {t.ID: {"Lunch": "No", "Dinner": "No", "Box Lunch": "No"} for t in tripulantes}

        # Actualizar las reservas con "Sí" para los tripulantes con reserva en la ciudad
        for restaurante in restaurantes:
            tripulante_id = restaurante.Tripulante_ID
            tipo_comida = tipo_comida_map.get(restaurante.Tipo_Comida, None)  # Traducir el tipo de comida

            if tipo_comida and tipo_comida in reservas[tripulante_id]:  # Asegurar que el tipo de comida existe en las claves
                reservas[tripulante_id][tipo_comida] = "Sí"
        ...
        for tripulante in tripulantes:
            resultados.append({
                ...
                "Lunch": reservas[tripulante.ID]["Lunch"],
                "Invoice Lunch": "",
                "Dinner": reservas[tripulante.ID]["Dinner"],
                "Invoice Dinner": "",
                "Box Lunch": reservas[tripulante.ID]["Box Lunch"],
                "Invoice Box Lunch": "",
            })
...
class RestaurantesLiquidarScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        # Label principal
        self.label = QLabel("RESTAURANTES LIQUIDAR")
    ...
    def actualizar_datos(self):
        ...
        if ciudad_seleccionada.lower() != "ciudad":
            self.label.setText(f"RESTAURANTES LIQUIDAR EN {ciudad_seleccionada.upper()}")  # Actualiza el label
        else:
            self.label.setText(f"RESTAURANTES LIQUIDAR")  # Actualiza el label
    ...
    def generar_reporte_liquidar(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "Guardar Reporte",
            f"liquidar_restaurantes_{vessel}_{datetime.now().strftime('%Y-%m-%d')}.xlsx",
    ...
    def generar_reporte_excel(self, df, output_file, vessel, fecha_eta, puerto, ciudad, tipo_crew):
        ...
        try:
            ...
            with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
                ...
                subtitulo = f"CREW MEALS AT {ciudad.upper()} - {tipo_crew} SIGNERS"
                ...
                for col_idx, col in enumerate(df.columns, start=1):
                    ...
                    if "Invoice" in col:  # Unificar encabezados de columnas Invoice
                        cell.value = "Invoice"
                    else:
                        cell.value = col
                ...
                # Colores para las celdas con datos de comida
                fill_pastel = PatternFill(start_color="ffc133", end_color="ffc133", fill_type="solid")  # Color pastel

                # Aplicar colores y eliminar "Sí/No" en las celdas de Lunch, Dinner y Box Lunch
                for row_idx, row in enumerate(df.itertuples(index=False), start=5):  # Las filas de datos comienzan en la fila 5
                    for col_name, col_value in zip(df.columns, row):
                        col_idx = df.columns.get_loc(col_name) + 1
                        cell = sheet.cell(row=row_idx, column=col_idx)

                        if col_name in ["Lunch", "Dinner", "Box Lunch"]:
                            if col_value == "Sí":  # Si tiene la comida, pintar la celda
                                cell.fill = fill_pastel
                                cell.value = ""  # Eliminar el texto "Sí"
                            else:
                                cell.value = ""  # Eliminar el texto "No"
