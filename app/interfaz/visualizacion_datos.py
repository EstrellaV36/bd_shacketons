...
class VisualizacionDatosScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        ciudades = self.controller.get_city_list()
    ...
    def get_city_list(self):
        ...
        try:
            ...
            ciudades = session.query(Buque.ciudad).distinct()
...
class VisualizacionDatosScreen(QWidget):
    ...
    def get_city_list(self):
        ...
        try:
            session = get_db_session()
            # Consulta para obtener las ciudades únicas asociadas a los buques
            ciudades = session.query(Buque.ciudad).distinct().all()
            
            # Eliminar duplicados y filtrar valores nulos
            ciudades_unicas = sorted({ciudad[0] for ciudad in ciudades if ciudad[0]})
